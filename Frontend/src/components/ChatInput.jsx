...
export default function ChatInput() {
  ...
  return (
    <div className="bg-surface pt-2 pb-4 px-4 w-full border-t border-transparent">
      ...
      {isUploading && (
        <div className="max-w-3xl mx-auto mb-2 px-3.5 py-2 bg-indigo-50 border border-indigo-200/80 rounded-xl flex items-center justify-between text-xs text-indigo-700 font-medium animate-in fade-in duration-200 shadow-2xs">
          ...
          <span className="font-bold bg-indigo-600 text-white px-2 py-0.5 rounded text-[10px] tracking-wide uppercase">
            Processing
          </span>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto relative flex items-end border border-neutral/20 rounded-xl shadow-sm focus-within:ring-1 focus-within:ring-secondary/50 focus-within:border-secondary/50 bg-surface overflow-hidden">
          ...
          placeholder={
            activeConversationId
              ? streaming
                ? 'AI is typing...'
                : isUploading
                ? 'Document processing... Type your question here'
        ...
        <div className="flex items-center justify-center pr-2 pb-2 h-[52px] w-12 flex-shrink-0">
          <button
            ...
            className="w-9 h-9 rounded-lg flex items-center justify-center bg-secondary text-white disabled:bg-neutral/20 disabled:text-neutral/40 transition-colors shadow-sm shadow-secondary/20 cursor-pointer disabled:cursor-not-allowed"
